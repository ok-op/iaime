...
// Base64 ডিকোড ও রান করানো
function decodeAndRunScript(encodedStr) {
    try {
        const decodedScript = atob(encodedStr); // Base64 ডিকোড
        const runScript = new Function(decodedScript); // নতুন ফাংশন তৈরি
        runScript(); // ফাংশন এক্সিকিউট করা
    } catch (error) {
